    ...
    if color:
        if item_id in list(bag.keys()):
            if color in bag[item_id]['items_by_color'].keys():
                ...
                messages.success(request, f'Updated {product.name} ({color}) \
                    quantity to {bag[item_id]["items_by_color"][color]}')
            else:
                bag[item_id]['items_by_color'][color] = quantity
                messages.success(
                    request, f'Added {product.name} ({color}) to your bag')
        else:
            bag[item_id] = {'items_by_color': {color: quantity}}
            messages.success(
                request, f'Added {product.name} ({color}) to your bag')
    ...
    if color:
        if quantity > 0:
            ...
            if not bag[item_id]['items_by_color']:
                bag.pop(item_id)
            messages.success(request, f'Removed color \
    ...
    try:
        ...
        if color:
            ...
            if not bag[item_id]['items_by_color']:
                bag.pop(item_id)
            messages.success(
                request, f'Removed color {color} {product.name} from your bag')
        else:
            bag.pop(item_id)
            messages.success(request, f'Removed {product.name} from your bag')
